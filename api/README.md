...
Для регистрации обязательно передавать согласие с условиями:
...
Если `terms_accepted=false`, API вернет `400 Bad Request` и откажет в регистрации.
...
- `GET /categories` - Список всех доступных категорий
  - Возвращает отсортированный массив уникальных категорий из всех мероприятий
  - Ответ:
    ```json
    ["выставка", "концерт", "спорт", "театр"]
    ```
